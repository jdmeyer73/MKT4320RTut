...
* The [airlineca.rdata](Data/airlineca.rdata) is used. Load it now.  Because of how the data is arranged, the training and holdout samples are both included in the file: `train.yog` and `test.yog`.
    ```{r}
    # You may need to change the directory
    load("Data/airlineca.rdata")
    ```
...
* [tradca.R](tradca.R) produces overal part-worth plots for each attribute, and importance plot, and a table with case-level part-worths and importances
* [cademo.R](cademo.R) produces correlation matrices and importance regressions from a dataframe of case-level part-worths/importances that has been appended with demographics
* [capred.R](capred.R) compares two profiles specified by the user
...
```{r}
source("cademo.R")   # Load the function'
...
```{r}
source("capred.R")   # Load the function'
