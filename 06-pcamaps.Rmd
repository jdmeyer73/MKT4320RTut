...
Base R is typically sufficient for performing the basics of principal components analysis, but to get some of the outputs required more easily and more efficiently, I have created a user defined function.  You should download this function and save it in your working directory.

* [pcaex.R](pcaex.R) provides eigenvalue table, scree plot, unrotated and rotated factor loading tables, and the principal components R object
...
* For principal components analysis (PCA), we need to pass a data frame to the user defined function containing the variables to be used.
    * Although we can pass a dataframe with additional variables, I find it easier to create a new dataframe with only those variables we want to include ( and maybe a preference and group/brand variable, see below)
...
* For this tutorial, we will perform a PCA using the `greekbrands` dataframe
    * We will use only the following attributes:
        ...
        * The data also has a preference variable, $pref$ and a brand variable, $brand$
...
* The `pcaex.R` user defined function can produce the results with one or two passes of the function
...
* Examples:
    * All components
    ```{r, messages=FALSE}
    # pcaex.R need to be "sourced"
    gb.all <- pcaex(pcadata,    # PCA data created earlier
                    group="brand",   # Grouping variable
                    pref="pref")    # Preference variable
    # Do not include 'comp' to get all components
    # Call gb.all$table to get eigenvalue table
    gb.all$table
    # Call gb.all$plot to get scree plot
    gb.all$plot
    ```
    * Two components
    ```{r}
    gb.2comp <- pcaex(pcadata,    # PCA data created earlier
                    group="brand",   # Grouping variable
                    pref="pref",    # Preference variable
                    comp=2)   # Request 2 components

    # Call gb.2comp$unrotated to get unrotated factor loadings
    gb.2comp$unrotated
    # Call gb.2comp$rotated to get rotated factor loadings
    gb.2comp$rotated
    ```
