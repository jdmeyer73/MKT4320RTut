...
* Usage: `or_table(MODEL, DIGITS, CONF)`
    ...
    * `CONF` is the confidence interval level (e.g., 95 = 95%)
    ``` {r t4or1, cache=TRUE}
    flextable(           # Wrapping it with 'flextable' for nicer output; not required
...
* Margin plots are done in much the same way as with [linear regression margin plots](#linregmarginplots)
    ...
    # Create data frame with values to be plotted with 'age' as focal
    age.pred <- data.frame(predictorEffect("age", model))
    ...
    # Create data frame with values to be plotted with 'gender' as focal
    g.pred <- data.frame(predictorEffect("gender", model))
    
    # Create plot and assign to 'p2'
    p2 <- g.pred %>%
