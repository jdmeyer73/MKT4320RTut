...
* To get standardized beta coefficients, a user defined function is created
* This function can be downloaded ([lm_beta.R]("lm_beta.R")) and saved in the working direction, and then sourced.
    ```{r t3lm4, cache=TRUE}
    source("lm_beta.R")
    ```
* Or the function can be ran in the current environment
    ```{r t3lm5, cache=TRUE}
...
# Function from lm.beta package
{
   ...
   return(object)
    ...
    ```
* Either way, the function `lm_beta(model)` take the stored results of the `lm()` model, and returns an vector with the standardized beta coefficients
    ```{r t3lm6, cache=TRUE}
    lm_beta(model1)  # Send previously saved 'model1' results to function
    ```
<caption>(\#tab:3lm6) Standardized regression coefficients</caption>    
